...
locals {
  ...
  create_lambdas_flag = try(var.create_lambdas, false)
...
resource "aws_lambda_permission" "fryrank_api_lambda_permission" {
  # Only create Lambda permissions when lambda creation is enabled.
...
resource "aws_lambda_function" "fryrank_api_lambdas" {
  ...
  # Only create Lambda function resources when lambda creation is enabled.
  # When disabled, the for_each becomes an empty map and no functions are created.
  for_each      = local.create_lambdas_flag ? local.lambda_functions : {}
